...
| lançador de harness (porta, V2-T28, assinatura estendida pela V2-T35 itens 2/4) | `HarnessLauncher` (`core/ports.ts`) — `open(cwd, addDirs, sessionId, systemPromptAppend)`, implementada por `adapters/harness/index.ts#ClaudeHarnessLauncher`, reusando `adapters/resumption/spawn-interactive.ts#runInteractive`/`env.ts#buildResumptionEnv` (D-017) — o mesmo `stdio: 'inherit'` que a retomada interativa já usa (docs/spikes/H-retomada-interativa.md), porque `open` também é uma sessão interativa de verdade, nunca headless. `adapters/harness/args.ts#buildOpenArgs` termina a lista de `--add-dir` com `--` explícito, sempre que houver algum diretório — o gotcha que `docs/spikes/N-adocao-de-sessao.md` mediu; `--session-id`/`--append-system-prompt` entram ANTES do `--add-dir`, de propósito, para nunca ficarem adjacentes à lista variádica |
| aviso de lock legível e sessão avisada (V2-T35) | item 1: `application/project-open.ts#openProject` pausa quando `lock.kind === 'readOnly'` e pede confirmação por `OpenProjectCallbacks.confirmReadOnlyOpen` (`ConfirmReadOnlyOpen`, três respostas nunca achatadas — `proceed`/`decline`/`unavailable`, D-024) antes de lançar o harness; sem terminal interativo, `unavailable` — `open` recusa dizendo o porquê, nunca abre sem que ninguém tenha visto o aviso. Implementada em `cli/project-command.ts#makeReadOnlyOpenConfirmer`, mesmo padrão `node:readline/promises` de `start-day-command.ts`, mas SEM resposta padrão silenciosa. Item 2: o mesmo texto do aviso (`core/project-lock-message.ts#formatProjectLockWarningLines`, movido de `cli/` para `core/` nesta tarefa para servir aos dois lados) vira `--append-system-prompt` do `claude` lançado — medido que a flag entrega a uma sessão FRESCA (nunca `--resume`, cujo "não entrega" é da Q-069). Item 3: ao sair, `formatOpenedReport` repete o aviso e mostra `OpenProjectResult.opened.finalLockStatus` (`ProjectLockStatus`, lido de novo DEPOIS do harness fechar — nunca o snapshot pré-lançamento). Item 4: ver a linha "identidade da sessão" abaixo |
| ganchos de git do espaço de trabalho (V2-T34 item 1, D-047 item 5) | `core/workspace-hooks.ts#buildCommitMsgHookScript(nodePath, cliEntryPath, env?)` — o texto do gancho `commit-msg` (um script `#!/bin/sh`, único em toda plataforma: o git do Windows sempre roda ganchos pelo `sh.exe` que ele mesmo empacota), chamando de volta `"$nodePath" "$cliEntryPath" project verify-commit "$1"` por CAMINHO ABSOLUTO (nunca `PATH`). Instalado por `application/workspace-hooks.ts#ensureWorkspaceHooksInstalled` em `application/workspace.ts#createProject` (na criação) e no início de `application/project-open.ts#openProject` (a cada `open` — "um gancho apagado volta sozinho"), via `WorkspaceRepository.installCommitMsgHook` (`adapters/workspace/index.ts`, escreve `<root>/.git/hooks/commit-msg` com o bit executável). A decisão pura é `core/workspace-commit-guard.ts#decideCommitGuard` — acrescenta os trailers que faltam (`CLAUDE_CODE_SESSION_ID` do ambiente ou, sem ela, o `sessionId` do lock do projeto tocado; sem nenhum dos dois, `unknown`) e recusa: mais de um projeto no mesmo commit, o arquivo `.seeya-lock` staged, ou um commit num projeto cujo lock está com OUTRA sessão viva (`ProcessControl.isAlive`) — um trailer já escrito que contradiz o que o gancho sabe também é recusado. A orquestração que junta os fatos (arquivos staged via `WorkspaceRepository.listStagedFiles`, a mensagem via a porta `CommitMessageFile`, o lock) é `application/verify-commit.ts#verifyCommit`, exposta como `seeya project verify-commit <messageFile>` (`cli/project-command.ts#runProjectVerifyCommitCommand`) — o único chamador é o próprio gancho. Executado de verdade (não só simulado) em `tests/integration/workspace/commit-msg-hook.test.ts`, incluindo um cenário com processo filho real para a checagem de vivacidade. **Revisão do PO — verificador ausente (item 2 do aceite):** o script agora testa `[ -f "$nodePath" ] && [ -f "$cliEntryPath" ]` antes do `exec` — um caminho gravado num `open` anterior que deixou de existir (seeya reinstalado/movido) recusa com uma mensagem nomeando o caminho que falta e `seeya project open <id>` como o conserto, nunca o erro cru do `sh` de antes; provado com um `git commit` real contra um `nodePath` inexistente (`commit-msg-hook.test.ts`'s own "missing verifier" case). **Revisão do PO — `ELECTRON_RUN_AS_NODE` na CLI (item 1 do aceite):** `packages/cli/src/composition.ts#resolveCliHookEnv(electronVersion)` (puro, parametrizado por `process.versions.electron`) devolve `{ ELECTRON_RUN_AS_NODE: '1' }` quando a própria CLI que está gerando o gancho roda sob Electron (o `seeya` instalado, `bin/seeya.cmd`/`/usr/bin/seeya` apontando para o executável do app) — sem isso, `nodePath` seria o binário do Electron e o gancho, ao rodar, abriria a janela do app em vez de verificar o commit. `ProjectContext.hookEnv` carrega o resultado para `open`/`create`; a janela já fazia isso sem condicional (`packages/app/src/composition/index.ts#projectHookIdentity`, sempre Electron). **Defeito de produção — `app.asar` (revisão do PO, 2026-09-25, achado no uso real em Ubuntu):** a checagem `[ -f "$cliEntryPath" ]` testava o caminho INTEIRO mesmo quando `cliEntryPath` mora dentro de um `app.asar` empacotado (`/opt/seeya/resources/app.asar/node_modules/@seeya-ai/cli/dist/index.js`) — para o shell, `app.asar` é um arquivo só, o caminho interno nunca existe, e o gancho recusava TODO commit no espaço de trabalho como "verificador ausente". `core/hook-verifier-path.ts#resolveHookVerifierCheckPath` (pura) trunca o caminho no próprio `.asar` só para fins da checagem `-f`; o `exec` continua com o `cliEntryPath` completo (é o que o Electron empacotado sabe ler). Provado com um `git commit` real contra um arquivo `.asar` de verdade em disco (`commit-msg-hook.test.ts`'s own "real .asar FILE" case). **Defeito de produção — o próprio `seeya` recusado pelo próprio gancho (revisão do PO, 2026-09-25, achado logo depois da correção do `app.asar`):** `decideSessionConflict` só aceitava `lock.sessionId === currentSessionId` — todo commit que o `seeya` faz segurando o lock do projeto (a sobra do `open`, a adoção, `remove`/`remove-repo`/`revert-adoption`) nunca roda dentro de uma sessão cujo `CLAUDE_CODE_SESSION_ID` bata com o `sessionId` do lock (que é um id recém-gerado para uma sessão que ainda nem começou, ou está ausente) — e um lock com `sessionId` ausente nunca "batia" com um `currentSessionId` também ausente (`undefined !== undefined` é `false`, D-025 cortando do jeito errado aqui). Ver `identificação do processo dono do lock` abaixo |
| identificação do processo dono do lock (V2-T34 hotfix, revisão do PO 2026-09-25) | `core/lock-holder-process.ts#LockHolderProcess` (`{ pid, procStart }`, mesmo par que `.seeya-lock` já grava) — uma SEGUNDA forma de autorização em `core/workspace-commit-guard.ts#decideSessionConflict`, ao lado da sessão: um commit é aceito quando `currentProcess` bate com `pid`/`procStart` do lock, mesmo sem nenhuma sessão em comum. Carregado pelas duas pontas do processo `git commit` por `SEEYA_LOCK_HOLDER_PID`/`SEEYA_LOCK_HOLDER_PROC_START` (`adapters/workspace/lock-holder-env.ts#buildLockHolderEnv`/`readLockHolderProcess`, mesmo padrão de `adapters/process/daemon-launch.ts#DAEMON_CHILD_ENV_VAR` — a constante mora com quem escreve o env real, a raiz de composição que lê importa pelo subcaminho público). `WorkspaceRepository.commitAll`/`revertCommits` ganharam um `lockHolder?` opcional (`core/ports.ts`), passado por `application/project-open.ts` (a sobra do `open`), `project-adopt.ts`, `project-remove.ts`, `project-remove-repo.ts` e `project-revert-adoption.ts` com o próprio `deps.pid`/`deps.procStart` de cada fluxo — nunca por `createProject`/`addRepository`, que não tomam lock algum. **Item 2 do aceite — o trailer que o próprio `seeya` já escreveu vale como está:** autorizado por processo, `decideTrailers` (`trustExistingSessionTrailer`) aceita um `Seeya-Session-Id` já presente na mensagem sem recomputar contra o `sessionId` do lock (que descreve o lock, não quem escreveu ESTE commit) — sem isso, a sobra do `open` (trailer deliberadamente `unknown`) seria recusada por "contradizer" o `launchedSessionId` do lock. **Item 3 — onde este guarda-corpo termina:** as duas variáveis podem ser forjadas por quem quiser rodar um processo com o pid/procStart certos — cobre o descuido, não o contorno deliberado, o mesmo limite que já vale para `CLAUDE_CODE_SESSION_ID` forjada. **Medido, não simulado (item 4):** `tests/integration/workspace/commit-msg-hook.test.ts`'s own "same-process lock-holder authorization" — os cinco fluxos que commitam segurando lock, gancho de verdade, CLI compilada de verdade, sem `CLAUDE_CODE_SESSION_ID` no ambiente e com uma OUTRA sessão no ambiente, todos aceitos; o caso proibido (outra sessão viva, processo errado) continua recusado. **Achado medido ao escrever o teste:** o commit do `removeProject` NUNCA precisou desta autorização — `removeProjectDirectory` apaga o diretório do projeto (e o `.seeya-lock` que mora dentro dele) ANTES do commit, então o gancho já vê `lock === null` e cai no mesmo caminho de `createProject`/`addRepository`, sem checagem de sessão nenhuma |
| gancho do harness (V2-T34 item 2) | `core/harness-hook-config.ts#buildHarnessSettingsJson(nodePath, cliEntryPath, env?)` — **revisado pelo PO (itens 2/3 do aceite): regenerado a cada `open`, nunca commitado, e chama o próprio `seeya` por caminho absoluto, nunca `node`/PATH.** `.claude/settings.json` (`HARNESS_SETTINGS_RELATIVE_PATH`) registra um hook `PreToolUse` no `Bash` cujo `command` é uma única expressão de shell (medido que o Claude Code executa `command` por um shell de verdade, não como argv literal: uma sessão descartável com `command: "FOO=bar123 node ...".mjs"` leu `process.env.FOO === 'bar123'` de volta) que testa `[ -f "$nodePath" ] && [ -f "$cliEntryPath" ]` e, só então, roda `"$nodePath" "$cliEntryPath" project verify-bash-command` (o mesmo padrão `KEY=value` de `ELECTRON_RUN_AS_NODE=1` do gancho de git quando aplicável); faltando o verificador, recusa com a mesma mensagem clara do gancho de git, mas com `exit 2` (o único código que bloqueia um `PreToolUse`, nunca `exit 1` — qualquer outro código é só aviso, o defeito original: o script antigo dependia de `node` no PATH e falhava aberto). A decisão pura é `decideBashCommandGuard` (`--no-verify`/`hooksPath`), aplicada de verdade por `seeya project verify-bash-command` (`cli/project-command.ts#runProjectVerifyBashCommandCommand`, lê o payload `PreToolUse` do stdin via `adapters/harness/bash-command-hook-payload-schema.ts#parseBashCommandFromHookPayload`, zod, tolerante). **Não é mais escrito pelo esqueleto do projeto** (`core/project-skeleton.ts` não inclui mais `.claude/`) — `application/harness-hook.ts#ensureHarnessHookInstalled` escreve `<root>/<projectId>/.claude/settings.json` a cada `openProject`, junto do gancho de git; `WorkspaceRepository.installHarnessHook` (nova) é quem grava, e a `.gitignore` do espaço de trabalho ganhou o padrão `**/.claude/` (`adapters/workspace/index.ts#IGNORED_WORKSPACE_PATTERNS` — precisa do prefixo `**` porque um padrão com `/` sem ele só vale na raiz do `.gitignore`, confirmado com `git status --porcelain --ignored=matching` real). **Medido de ponta a ponta, não só simulado:** duas sessões descartáveis reais, o `packages/cli/dist/index.js` compilado de verdade — pedir para rodar `git commit --no-verify -m test` foi bloqueado ("The git hook blocked this command..."); um `echo` comum passou. **Medido, não presumido: esta camada NÃO protege `seeya project adopt`** — o Claude Code só lê `.claude/settings.json` do `cwd` da sessão, nunca de um diretório liberado só por `--add-dir` (`code.claude.com/docs/en/permissions`: "doesn't discover most `.claude/` configuration from these directories"; confirmado também com uma sessão descartável real: um hook em `cwd` disparou, o mesmo hook num diretório só `--add-dir`'d não disparou). A adoção continua coberta pelos ganchos de git (que rodam dentro do próprio repositório do espaço de trabalho, não do `cwd` da sessão). **Defeito de produção — `app.asar` (revisão do PO, 2026-09-25):** mesmo defeito e mesma correção do gancho de git acima — a checagem do `cliEntryPath` agora passa por `core/hook-verifier-path.ts#resolveHookVerifierCheckPath` antes do `-f`. Provado com um payload `PreToolUse` real, por um shell de verdade (o mesmo `sh.exe` que o Git para Windows empacota, resolvido por `git --exec-path`, já que ganchos do Claude Code nunca herdam o stdin de um `git commit` para reusar aquele truque), contra um `.asar` real em disco (`tests/integration/harness/harness-hook-command.test.ts`) |
| captura do stderr do git (V2-T34, revisão do PO 2026-09-25) | `adapters/git/run-git.ts#GitCommandResult`'s own `stderr` (novo campo, sempre presente quando `ran: true`) — antes, `runGit` descartava o stderr do git inteiro (`stdio: ['ignore', 'pipe', 'ignore']`); o motivo real de uma recusa (o gancho de commit, por exemplo) desaparecia, e quem chamou só via "exit 1". `adapters/workspace/index.ts#commitAll` é o único ponto que grava um commit de verdade neste projeto — seu erro agora inclui `commit.stderr.trim()`, não só o código de saída |
| liberação garantida do lock do projeto (V2-T34, revisão do PO 2026-09-25 — defeito de produção) | Achado do mantenedor: uma adoção real cujo commit falhou (o gancho de git recusou) travou o lock do projeto para sempre — `application/project-adopt.ts#adoptSession` tinha `releaseProjectLock` copiado à mão em cada ramo, e um `commitAll` que lançava pulava todos eles. Corrigido com `try`/`finally` — um `finally` único, sempre executado, em `adoptSession` e nos outros quatro fluxos que tomam o lock do projeto (`application/project-open.ts#openProject`, `project-remove.ts#removeProject`, `project-remove-repo.ts#removeRepository`, `project-revert-adoption.ts#revertAdoption`); `releaseProjectLock` já era idempotente (`core/project-lock.ts#decideProjectLockRelease` só libera um lock cujo `pid` é o do próprio processo), então o `finally` é seguro mesmo quando o caminho normal já liberou explicitamente antes |
| `commitFailed` da adoção (V2-T34, revisão do PO 2026-09-25 — defeito de produção) | `AdoptSessionResult`'s own `commitFailed` (`application/project-adopt.ts`, união discriminada, D-024) — `commitAll` recusado (o gancho de git, o mais comum) não propaga mais como exceção crua: `commitAdoption` captura e devolve `{ kind: 'commitFailed', reason }`, com `reason` sendo a mensagem de `commitAll` (já com o stderr do git, ver linha acima). Mesmo tratamento não destrutivo de `confirmationUnavailable`: os arquivos da cópia ficam no disco, a cópia continua registrada em `forks.json` como pendente, `adoptions.json` nunca ganha entrada. `cli/format-project.ts#formatAdoptSessionReport`/`packages/app/src/state/adopt-session-result.ts#formatAdoptSessionOutcomeText` mostram o motivo para quem chamou — nunca mais um silêncio que parecia sucesso |
| a janela nunca fecha em silêncio numa falha inesperada (V2-T34, revisão do PO 2026-09-25 — defeito de produção) | `electron/adopt-flow-view.ts`/`electron/projects-list-view.ts#triggerProjectOpen` — o `.then()` que espera `adoptSession`/`openProject` pela IPC não tinha `.catch()`: uma rejeição (antes desta tarefa, todo `commitAll` recusado por um gancho; depois dela, qualquer exceção genuinamente inesperada) deixava o diálogo de adoção parado em `idle` para sempre, ou o texto de resultado do `open` em branco — o achado do mantenedor ("o diálogo fechou como se tivesse dado certo"). Os dois agora têm `.catch()`, despachando o mesmo evento `resultReceived`/preenchendo o mesmo texto com uma mensagem de erro |
| diálogo de mudanças pendentes na janela (V2-T34 item 4, revisão do PO 2026-09-25 — item que faltou) | A janela nunca perguntava — todo `open` com sobra de sessão anterior caía direto no refuso `leftoverChangesConfirmationUnavailable` ("18 uncommitted change(s)... refusing to open without confirmation"), sem diálogo nenhum, porque `electron/project-ipc.ts` nunca passava `confirmLeftoverChanges` para `openProject`. `core/project-lock-message.ts#renderLeftoverChangesLines` (movida de `cli/format-project.ts`, mesmo texto, sem o sufixo `[c = commit now...]` do `readline`) é a mesma frase nos dois lados; `electron/project-leftover-changes-confirm-dialog-view.ts` é o diálogo novo (**Commit now**/**Continue without committing**), pelos canais `confirmLeftoverChangesOpenRequest`/`answerLeftoverChangesOpenConfirm` (`ipc/channels.ts`) — mesmo padrão de `PendingConfirmations` que `project-lock-confirm-dialog-view.ts` já usa para a pergunta do lock. `electron/dialog-lines.ts#renderDialogLines` (extraída de `adopt-flow-view.ts`, reusada aqui) é quem desenha uma linha por arquivo mudado |
| auditoria (V2-T34 item 3, D-047 item 5) | `seeya project audit <id>` (`cli/project-command.ts#runProjectAuditCommand`) — `application/project-audit.ts#auditProject`: lê o marcador `ProjectAuditMarker` (porta nova, `.seeya-audit` dentro do projeto — mesma disciplina de nunca-commitado do `.seeya-lock`), pega todo commit desde ali (ou a história inteira, sem marcador) via `WorkspaceRepository.listCommitsForAudit` (lista de arquivos SEM escopo por projeto, ao contrário de `findCommitsAfter` — é o que permite `core/project-audit.ts#auditCommits` detectar um commit que também tocou outro projeto), decide o que escapou (trailer ausente/errado, toca outro projeto, inclui o lock) e grava o novo marcador. Chamada também por `application/project-open.ts#openProject`, ANTES de tomar o lock — o relatório sai por `OpenProjectCallbacks.onBeforeLaunch`'s own `audit`. **Onde para (item 7):** só enxerga o que já está no histórico do git — nunca prova que um `Seeya-Session-Id` presente e sintaticamente válido foi realmente escrito por aquela sessão |
| regras de trabalho entregues à sessão (V2-T34 item 5) | `core/project-working-rules.ts#buildProjectWorkingRulesText(projectId)` — texto curto (< 4000 caracteres, bem abaixo do teto real de D-015/Q-069), entregue em TODO `open` (nunca só quando há algo a avisar, ao contrário do aviso de lock) via o MESMO `--append-system-prompt` que o aviso de lock já usa (`application/project-open.ts#systemPromptAppendFor`, que agora sempre devolve texto, nunca `null`) — id do projeto, commitar no caminho sem perguntar, trailers automáticos, um projeto por commit, nunca o `.seeya-lock`, editar os canônicos na hora da decisão (`journal/` como rascunho), segredo por caminho. **Limite conhecido:** a Q-069 já mediu que `--append-system-prompt` não chega a uma sessão RETOMADA — a adoção (que usa `--resume`) não recebe este texto; a instrução dela própria já cobre o que precisa |
| sobra de sessão anterior no próximo `open` (V2-T34 item 4) | `application/project-open.ts#handleLeftoverChanges` — só quando ESTA tentativa tomou o lock (nunca num `open` para leitura) e `WorkspaceRepository.listChangedFiles` não está vazio: pergunta por `OpenProjectCallbacks.confirmLeftoverChanges` (`ConfirmLeftoverChanges`, três respostas nunca achatadas — `commitNow`/`proceedWithoutCommitting`/`unavailable`, D-024). `commitNow` commita com `Seeya-Session-Id: unknown` (D-025 — não dá para afirmar de quem foi); `proceedWithoutCommitting` segue e soma a lista de pendências ao `--append-system-prompt` da sessão nova; sem confirmador, `OpenProjectResult`'s own `leftoverChangesConfirmationUnavailable` (o lock já tomado é liberado antes de recusar). Implementada em `cli/project-command.ts#makeLeftoverChangesConfirmer`, mesmo `readline` compartilhado de `confirmReadOnlyOpen` |
| adoção de sessão (V2-T29, D-045 item 4, D-047 item 6) | `seeya project adopt <sessão> <projectId>` (o `<sessão>` resolvido pelo nome que `seeya sessions` mostra, `cli/session-reference.ts#resolveSessionReference`, nunca id cru) — `application/project-adopt.ts#adoptSession`. Roda numa CÓPIA (`--resume` + `--fork-session` + `--session-id <id gerado pelo seeya>`, `adapters/harness/adopt-args.ts#buildAdoptArgs`), resumida interativamente no diretório ORIGINAL da sessão — decisão do mantenedor, 2026-09-24: o Claude Code carrega pelo diretório de trabalho o `CLAUDE.md`/a memória automática/configuração/skills locais dali, e é isso que a cópia precisa ter; aberta no projeto, ficaria só com o histórico (o trabalho do dia a dia no projeto continua sendo pelo `project open`, um momento separado) — com `--add-dir` liberando só o diretório do projeto e **sem nenhum `--permission-mode`** — a pessoa aprova cada escrita na hora (D-047: a decisão do mantenedor por interativo, nunca headless). Confirmado com uma sessão descartável que as três flags funcionam juntas e a original não muda uma linha (`docs/QUESTOES.md` Q-090). Sessão viva (`alive`/`idle`) é recusada — retomar abriria uma segunda cópia; sessão já adotada (`adoptions.json`) também |
| instrução da adoção (V2-T29, caminho absoluto e memória local desde a correção do aceite) | `adapters/harness/adopt-instruction.ts#buildAdoptionInstruction(projectDir)` — texto fixo e curto, função do diretório do projeto (era `ADOPTION_INSTRUCTION`, uma constante, até o aceite do mantenedor achar o defeito: sem o caminho, a sessão lia "o diretório do projeto" como o próprio `cwd` — `docs/QUESTOES.md` Q-093). Primeiro turno da sessão retomada (argumento posicional, dentro do teto medido em D-015): nomeia o diretório logo na primeira frase; pede que a sessão traga para o projeto o que, das instruções e da memória disponíveis no PRÓPRIO diretório dela, pertence a este trabalho — e deixe de fora o que é de outro assunto (item acrescentado pelo mantenedor no mesmo dia, junto da correção; confirmado com um `CLAUDE.md` descartável e um fato inventado que apareceu no projeto adotado, `docs/QUESTOES.md` Q-094); nomeia cada arquivo pelo caminho ABSOLUTO a partir do projeto (`<projectDir>/AGENTS.md`, `<projectDir>/INDEX.md`, `<projectDir>/status`, `<projectDir>/decisions`, `<projectDir>/context/know-how.md` — nunca a palavra "memory" desacompanhada, achado do `docs/spikes/N-adocao-de-sessao.md`), pede caminho de segredo nunca valor, e pede que o incerto fique marcado como incerto. `adapters/harness/adopt-args.ts#buildAdoptArgs` e `SessionAdoptionLauncher.adopt` (`core/ports.ts`) recebem `projectDir: string` (nunca mais uma lista `addDirs` — este fluxo só libera um diretório, D-024) |
| confirmação antes do lançamento da adoção (V2-T29, itens 2/6/8/9 do aceite corrigido) | `application/project-adopt.ts#AdoptSessionCallbacks.confirmLaunch` (`ConfirmAdoptionLaunch`, três respostas nunca achatadas — `proceed`/`decline`/`unavailable`, D-024) — perguntada ANTES de o projeto existir, o lock ser tomado ou o fork ser registrado (mesmo "explica e espera" que `confirmReadOnlyOpen` já dá a um projeto travado, V2-T35 item 1, agora para o problema que o segundo aceite do mantenedor achou: o `claude` assume a tela logo depois, então o que foi dito antes precisa ser lido, não só impresso). Texto: `cli/format-project.ts#renderAdoptionLaunchConfirmation` — diz onde a cópia abre e por quê (instruções e memória locais do diretório original), onde o projeto fica, e a sugestão de continuar depois por `seeya project open <projectId>`; pergunta `[Y/n]`, em branco = continuar (`parseAdoptionLaunchConfirmation`, o ÚNICO "sim" silencioso deste projeto — decisão deliberada: a pergunta guarda a própria ação que `adopt` foi chamado para fazer, ao contrário de toda outra confirmação daqui, que tem branco = não). Recusar (`launchConfirmationDeclined`) ou não ter terminal (`launchConfirmationUnavailable`) não cria nada — nem projeto, nem lock, nem registro em `forks.json`. Implementada em `cli/project-command.ts#makeAdoptionLaunchConfirmer`, sobre o MESMO `readline.Interface` que `makeAdoptionCommitConfirmer` usa (`openConfirmationReader`) — medido que abrir uma segunda interface no mesmo stream depois de fechar a primeira perde o que já estava em buffer e trava esperando entrada que nunca chega; por isso as duas perguntas de `adopt` compartilham uma interface por invocação, aberta e fechada uma vez só. Aceita, `formatAdoptSessionReport`'s own caso `adopted` repete a sugestão `seeya project open <projectId>` ao final (item 9) |
| saber-fazer do projeto (V2-T29) | `context/know-how.md` — nome fixado pelo PO em 2026-09-24, dentro do `context/` que o esqueleto do projeto já cria (V2-T27): ferramentas e skills usadas, caminhos de acesso, configuração por ambiente, convenções — o que a compactação apaga primeiro, segundo o relato do mantenedor. Só a instrução da adoção referencia o nome; ninguém no código grava neste arquivo além da própria sessão adotada |
| registro de forks separa captura de adoção (V2-T29, D-012) | Nenhuma mudança de schema em `forks.json`: uma cópia de adoção PENDENTE é registrada com `registerFork`/`unregisterFork` (`adapters/generation/fork-registration.ts`) exatamente como uma cópia de captura — escondida da descoberta, e apagada de graça por `forkCleanupDays` se abandonada. Aceita: `ForkRegistration.unregister` tira a entrada de `forks.json` (nunca mais escondida, nunca mais apagável por idade) e `adoptions.json` grava o registro — é essa ausência em `forks.json` mais presença em `adoptions.json` que "promove" o fork. Recusada antes do commit: `ForkCleanup.deleteFork` apaga o transcript (a mesma exceção do D-012, por `sessionId`, nunca por caminho) e `ForkRegistration.unregister` tira a entrada — nada fica registrado |
| `ForkRegistration` (porta, V2-T29) | `core/ports.ts` — `register`/`unregister` um `sessionId` em `forks.json`, para `application/project-adopt.ts` (que não pode importar `adapters/discovery/fork-registry.ts` direto, D-020). Implementada por `adapters/generation/fork-registration.ts#GenerationForkRegistration`, sobre as mesmas `registerFork`/`unregisterFork` que `deep-generator.ts` já chama direto |
| `SessionAdoptionLauncher` (porta, V2-T29) | `core/ports.ts` — irmã de `HarnessLauncher`: `adopt(originalCwd, addDirs, originalSessionId, forkSessionId)`, implementada por `adapters/harness/session-adoption.ts#ClaudeSessionAdoptionLauncher`, reusando o mesmo `runInteractive`/`buildResumptionEnv` (D-017) de `ClaudeHarnessLauncher`/`SessionResumer` |
| `WorkspaceRepository.listChangedFiles` (V2-T29) | `core/ports.ts` — `git status --porcelain --untracked-files=all`, escopado a um projeto só (a mesma técnica de `commitAll`, D-047 item 3); `application/project-adopt.ts#adoptSession` lê isto depois que a sessão fecha, para mostrar antes de perguntar "commita?" (item 4: "nada é commitado sem a pessoa") |
...
| identidade da sessão que segura o lock/commita (V2-T33, D-047; duas fontes desde a V2-T35 item 4) | Duas fontes, cada uma para o seu uso: **para `seeya project open`'s own lock**, `launchedSessionId` — o id que o PRÓPRIO `open` gera (`packages/cli/src/composition.ts#buildProjectOpenDeps`, `node:crypto#randomUUID`, geração de aleatoriedade por isso fora de `core/`/`application/`) e passa a `claude --session-id <id>`; vale sempre, mesmo quando `open` roda de dentro de outra sessão — a dona do lock é a sessão que o `open` abriu, nunca a que o chamou. **Para os demais comandos** (`create`/`add-repo`/os trailers de commit), continua `sessionId` — `process.env.CLAUDE_CODE_SESSION_ID` (a mesma variável da tabela de D-017), lido só na raiz de composição (`packages/cli/src/composition.ts`) e passado adiante como `string \| undefined`: ausente quando `seeya` roda fora de uma sessão do Claude Code (D-025, nunca inventado). Vira o trailer `Seeya-Session-Id` mesmo ausente — nesse caso o valor literal é `unknown`, nunca uma sessão inventada |
...
| `seeya project remove <id>` (V2-T32, item 1) | `application/project-remove.ts#removeProject` — toma o lock do projeto (D-047 item 8, mesmo padrão de `open`: recusa se outra sessão viva o segura, toma e solta no fim), pede confirmação (nome do projeto e `WorkspaceRepository.countProjectFiles`, `git ls-files` escopado) e, aceita, `WorkspaceRepository.removeProjectDirectory` + `commitAll` — nunca `-A`, então git já registra a remoção como parte do diff do próprio projeto. A saída sempre repete `WorkspaceRepository.currentCommit` (o `HEAD` de ANTES da remoção) como "como recuperar". Item 4: nunca apaga repositório associado, sessão ou transcript — nenhum deles fica dentro de `root/projectId`. Item 7: tira de `adoptions.json` toda adoção daquele projeto (as originais voltam a poder ser adotadas) e reporta quais eram — as cópias promovidas continuam intocadas |
| `seeya project remove-repo <id> <nome>` (V2-T32, item 2) | `application/project-remove-repo.ts#removeRepository` — mesmo lock (D-047 item 8), sem confirmação (precedente do próprio `add-repo`: reversível pelo histórico do espaço de trabalho). Tira o `AssociatedRepository` de `seeya.json` e commita; a entrada em `repository-map.json` só sai se nenhum OUTRO projeto ainda usa a mesma `RepositoryIdentity` (uma sem identidade é sempre exclusiva de um projeto, sai sempre) |
| `seeya project revert-adoption <id> [<sessão>]` (V2-T32, itens 3/5/6/8, D-047 item 4) | `application/project-revert-adoption.ts#revertAdoption` — `<sessão>` (opcional só quando o projeto tem uma adoção; obrigatória com mais de uma) casa por id exato ou prefixo contra `originalSessionId`/`forkSessionId` de `adoptions.json` (`core/adoption-registry.ts#selectProjectAdoption`; nunca por nome/`cwd` — Q-095 explica por quê). Acha os commits da cópia por `WorkspaceRepository.findSessionCommits` (trailer `Seeya-Session-Id`, D-047 item 4) e os posteriores por `findCommitsAfter`; `core/project-revert.ts#planAdoptionRevert` (puro) recusa se algum posterior de outra sessão tocou os mesmos arquivos, senão reverte do mais novo para o mais antigo num commit só (`WorkspaceRepository.revertCommits` — `git revert --no-commit` em sequência, aborta tudo no primeiro que não aplica limpo, nunca revert pela metade). Pede confirmação antes de reverter; feito, tira o registro de `adoptions.json` (a original volta a poder ser adotada) e decide a cópia (item 6): compara o `mtime` do transcript (`ForkCleanup.checkForkActivity`) com `adoptedAt` (`core/adopted-copy-growth.ts#decideAdoptedCopyGrowth` — Q-095 explica por que é `mtime`, não um tamanho de referência); sem escrita depois da adoção, apaga sem perguntar; escreveu depois (ou o transcript sumiu, D-025), pergunta com resposta padrão "manter" |
| agrupamento de sessão por projeto (V2-T30, item 1) | `packages/app/src/sidebar/project-sessions.ts#groupSessionsByProject` (pura) — casa sessão↔projeto só por evidência (D-025), nunca por repositório associado: `cwd` normalizado (`core/cwd-normalization.ts`) igual ao diretório do projeto, ou `sessionId` é o `AdoptionRecord.forkSessionId` registrado para aquele projeto (nunca o `originalSessionId`), ou `sessionId` é quem segura o `.seeya-lock` do projeto (quando conhecido — um lock sem `sessionId`, D-025, não casa com nada). Sem evidência nenhuma, a sessão cai em "Other sessions", nunca escondida nem adivinhada. `resolveAdoptEligibility` (mesmo arquivo) decide se uma sessão de "Other sessions" pode ganhar **Adopt…** — sessão viva (`alive`/`idle`) e sessão já adotada (`adoptions.json`) ficam desabilitadas com o motivo nomeado, D-024 |
| painel de projetos (V2-T30, item 1) | `state/projects-panel.ts#buildProjectsPanelData` — junta o agrupamento acima ao `ProjectLockStatus` de cada projeto (`@seeya-ai/engine/application/project-lock.js`, o mesmo que `seeya project show` usa) num `ProjectsPanelData` (`projects: ProjectPanelRow[]` + `otherSessions: ProjectPanelOtherSessionRow[]`) — texto do lock já pronto para a tela (`formatLockText`, exportada para `state/project-open-result.ts` reusar a mesma frase). `electron/project-ipc.ts#wireProjectIpc` lê `listProjects`/`readAdoptions`/um `.seeya-lock` por projeto a cada ciclo de 10s (reusa o `SidebarRow[]` que o ciclo já calcula, sem segunda descoberta) e de novo após cada ação de projeto; `CHANNELS.getProjectsPanel` (invoke) cobre a primeira pintura — achado medido numa janela real descartável: o primeiro tick do laço ambiente dispara antes de o renderer terminar de carregar e registrar `CHANNELS.projectsUpdate` (push), perdendo esse primeiro push, o mesmo mecanismo de `getTodayPanel`/`onTodayUpdate` |
| lateral recolhível (V2-T30, item 2) | `state/sidebar-collapse.ts` (`parseSidebarCollapsedPreference`/`encodeSidebarCollapsedPreference`, puras) — preferência de visualização, nunca `config.json`: `localStorage` do renderer, chave `seeya.sidebarCollapsed` (`SIDEBAR_COLLAPSED_STORAGE_KEY`), valor literal `'true'`/`'false'`. Leitura protegida (D-025): ausência, valor malformado ou `localStorage` lançando (janela privada, site data bloqueado) sempre abrem expandida, nunca recolhida por adivinhação. `electron/sidebar-collapse-view.ts#wireSidebarCollapse` é a fiação (botão sempre visível, `#sidebar.collapsed` no CSS) |
| lançadores de aba para `open`/`adopt` (V2-T30, itens 3/5) | `packages/app/src/resume/project-tab-launcher.ts` — `ProjectOpenTabLauncher` implementa `HarnessLauncher` e `ProjectAdoptTabLauncher` implementa `SessionAdoptionLauncher`, os dois reusando o `TabResumeOpener` que a V2-T4 já tem (`resume/tab-session-resumer.ts`) em vez de uma segunda interface — mesmo mecanismo de montar aba para um pty já lançado, nunca resume-específico. Ao contrário de `TabSessionResumer`, estes DOIS esperam o pty sair antes de resolver (`HarnessLauncher.open`/`SessionAdoptionLauncher.adopt`'s own contrato de bloquear até o harness fechar) — o que não bloqueia a janela é `electron/project-ipc.ts` nunca esperar essa promise antes de responder ao clique; a aba aparece de imediato pelo `CHANNELS.resumeTabOpened` já existente |
| máquina de estados do fluxo de adoção (V2-T30, item 5) | `state/adopt-panel.ts` (`AdoptPanelState`/`reduceAdoptPanel`, mesmo formato de `state/end-day-panel.ts`) — seis estados: `idle` / `pickProject` / `launchConfirm` / `commitConfirm` / `result`. **O diálogo fica fechado (`idle`) entre a resposta do `launchConfirm` e o próximo push** — de propósito: um `<dialog>` modal bloquearia a própria aba que a pessoa precisa usar enquanto o fork roda. Fiação em `electron/adopt-flow-view.ts` |
| texto compartilhado da adoção (V2-T30) | `core/project-adoption-message.ts` (novo módulo) — `renderAdoptionLaunchExplanationLines`/`renderAdoptionCommitChangedFilesLines`, movidas de `packages/cli/src/format-project.ts` (mesmo movimento que a V2-T35 já fez para o aviso do lock): `cli/format-project.ts#renderAdoptionLaunchConfirmation`/`renderAdoptionCommitConfirmation` agora só concatenam o sufixo de prompt (`[Y/n]`/`[y/N]`) por cima — saída da CLI byte-idêntica. A janela mostra as mesmas linhas nos diálogos de `adopt-flow-view.ts`, sem duplicar o texto |
| pergunta do lock legível compartilhada (V2-T30, item 3) | `core/project-lock-message.ts#renderReadOnlyOpenQuestion(heldBy)` — a mesma frase de `renderReadOnlyOpenConfirmation` da CLI (V2-T35), sem o sufixo `[y/N]` de `readline`; `cli/format-project.ts#renderReadOnlyOpenConfirmation` agora só concatena esse sufixo. A janela mostra a mesma frase em `electron/project-lock-confirm-dialog-view.ts`, com botões **Open read-only**/**Cancel** em vez de digitar y/n |
| canais de IPC da janela por projeto (V2-T30) | `ipc/channels.ts` — `getProjectsPanel` (invoke, primeira pintura) / `projectsUpdate` (push, ciclo de 10s e pós-ação); `createProject` (invoke); `openProject` (invoke, só resolve quando a aba fecha — nunca esperado pelo clique) / `confirmProjectLockOpenRequest` + `answerProjectLockOpenConfirm` (a pergunta do lock); `adoptSession` (invoke, mesma semântica de não bloquear) / `confirmAdoptionLaunchRequest` + `answerAdoptionLaunchConfirm` / `confirmAdoptionCommitRequest` + `answerAdoptionCommitConfirm` |
| rótulo do estado da sessão (V2-T52) | `core/session-state-label.ts#formatSessionStateLabel(state)` — o único lugar, CLI↔janela, que traduz `SessionState` para texto; o enum em disco/código continua exatamente `alive`/`idle`/`ended`/`unknown` (D-027), só `unknown` ganhou rótulo próprio, **"no running process"** — o achado do aceite da V2-T29/V2-T30: a sessão que a pessoa acabou de fechar mostrava `unknown` e lia como erro, quando o fato é só "sem PID para checar" (D-016/D-025). `cli/format-sessions.ts` e `packages/app/src/state/projects-panel.ts`/`session-search.ts` chamam esta função; nenhum lugar imprime `state`/`row.state` cru |
| busca direta por id, ignorando `relevanceHours` (V2-T55 item 1) | `adapters/discovery/session-id-lookup.ts#findSessionByIdPrefix`/`DiscoverySessionIdLookup`, atrás da porta nova `core/ports.ts#SessionIdLookup` (deliberadamente separada de `SessionProvider`, para nunca entrar no ciclo de 10s — ver o próprio docstring da porta) — varre `~/.claude/projects/**/*.jsonl` por PREFIXO do nome do arquivo, sem o corte de `relevanceHours` que `transcript-scan.ts` aplica; reusa `collectCandidateFiles`/`statMtimeMs`/`processTranscriptFile` de `transcript-scan.ts` (agora exportadas), nunca uma segunda varredura. Prefixo que casa com mais de uma sessão: `SessionIdLookupOutcome`'s own `ambiguous` (D-025, nunca escolhe). Só roda sob demanda: `seeya project adopt` (`cli/session-reference.ts#resolveSessionReferenceForAdoption`, fallback só quando a busca na janela de tempo falhou E o valor "parece um id", `core/session-id-shape.ts#looksLikeSessionIdReference`) e o campo de busca da janela (`electron/session-search-ipc.ts`, via `application/session-id-search.ts#findSessionByIdOrPrefix` contra um `sessionProvider.list()` fresco) |
| sessões agrupadas por diretório em "Other sessions" (V2-T55 item 2) | `sidebar/project-sessions.ts#groupOtherSessionsByDirectory` (pura, `cwd` normalizado) — uma linha por diretório com `sessionCount`, no lugar de uma linha por sessão; `state/projects-panel.ts#OtherSessionDirectoryPanelRow`/`ProjectsPanelData.otherSessionsByDirectory` (renomeado de `otherSessions`) |
...
| passo do instalador Windows por máquina, com horário e instância | `installer.log` — uma linha por passo (`seeyaLogWrite`/`seeyaRunLoggedCli`, `packages/app/build/installer.nsh`), rotulada `original`/`elevated`/`as-user`/`old-uninstaller` (`as-user`: `customInstall` da instância elevada religando o daemon através de `UAC_AsUser_Call`, código que roda de volta na instância `original`, sem o privilégio da elevada — por isso rótulo próprio); **não** um logger do `seeya` (o mesmo motivo de `autostart.log`, acima) — é o NSIS's own instalador escrevendo, com `FileWrite`/`${GetTime}` puros, antes de qualquer processo `seeya` existir; acrescenta, nunca sobrescreve, com teto de 256 KiB (`SEEYA_INSTALLER_LOG_MAX_BYTES`) | raiz de `~/.seeya/` (V2-T45 item 5) |
...
| marcador de auditoria do projeto (arquivo) | `.seeya-audit` / o hash do último commit já auditado (texto puro, uma linha) | dentro de cada `<projectId>/` do espaço de trabalho — mesma disciplina do `.seeya-lock` acima: nunca em `~/.seeya/`, nunca commitado (V2-T34, D-047 item 5) |
| registro de adoções aceitas | `adoptions.json` / `schemaVersion`, `adoptions` — cada item `originalSessionId`, `forkSessionId`, `projectId`, `adoptedAt` | raiz de `~/.seeya/` (V2-T29, D-047 item 6) |
...
`project` (`create` / `list` / `show`, V2-T27; `add-repo` / `open`, V2-T28; `adopt`, V2-T29;
`remove` / `remove-repo` / `revert-adoption`, V2-T32; `audit`, V2-T34 item 3; `verify-commit`,
V2-T34 item 1 — só o gancho de git do espaço de trabalho chama este último, nunca uma pessoa;
`verify-bash-command`, V2-T34 item 2 — só o gancho do harness chama este, nunca uma pessoa),
e `--session`, `--all`,
