...
| marcador de registro do protocolo (V2-T5b) | `~/.seeya/protocol-handler.json`, campo `registered` — `core/ports.ts#Storage.readProtocolHandlerRegistered`/`saveProtocolHandlerRegistered`; gravado por `packages/app/src/composition/index.ts` (via `electron/main.ts`) quando `app.setAsDefaultProtocolClient('seeya')` retorna sucesso; lido pelo backend de toast do Windows (`adapters/notification/windows-toast.ts`) antes de incluir `launch="seeya://open"` no toast — sem marcador, toast como antes desta tarefa (D-025) |
| notificador com verificação de protocolo (V2-T5b) | `adapters/notification/index.ts#buildNotifier(isProtocolHandlerRegistered, platform?)` — a variante do `notifier` singleton que injeta a leitura do marcador acima no `WindowsToastBackend`; só `packages/cli/src/composition.ts#buildDaemonContext` usa ("quem manda o toast é o daemon") — `buildEndDayContext`/a interface continuam com o `notifier` singleton sem essa checagem |
...
| marcador de registro do protocolo / registrado | `protocol-handler.json` / `registered` | raiz de `~/.seeya/` (V2-T5b) |
