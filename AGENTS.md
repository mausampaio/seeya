...
| adoção de sessão (V2-T29, D-045 item 4, D-047 item 6) | `seeya project adopt <sessão> <projectId>` (o `<sessão>` resolvido pelo nome que `seeya sessions` mostra, `cli/session-reference.ts#resolveSessionReference`, nunca id cru) — `application/project-adopt.ts#adoptSession`. Roda numa CÓPIA (`--resume` + `--fork-session` + `--session-id <id gerado pelo seeya>`, `adapters/harness/adopt-args.ts#buildAdoptArgs`), resumida interativamente no diretório ORIGINAL da sessão, com `--add-dir` liberando só o diretório do projeto e **sem nenhum `--permission-mode`** — a pessoa aprova cada escrita na hora (D-047: a decisão do mantenedor por interativo, nunca headless). Confirmado com uma sessão descartável que as três flags funcionam juntas e a original não muda uma linha (`docs/QUESTOES.md` Q-090). Sessão viva (`alive`/`idle`) é recusada — retomar abriria uma segunda cópia; sessão já adotada (`adoptions.json`) também |
| instrução da adoção (V2-T29) | `adapters/harness/adopt-instruction.ts#ADOPTION_INSTRUCTION` — texto fixo, curto, o primeiro turno da sessão retomada (argumento posicional, dentro do teto medido em D-015): nomeia os arquivos pelo caminho (`AGENTS.md`, `INDEX.md`, `status/`, `decisions/`, `context/know-how.md` — nunca a palavra "memory" desacompanhada, achado do `docs/spikes/N-adocao-de-sessao.md`), pede caminho de segredo nunca valor, e pede que o incerto fique marcado como incerto |
