/**
 * The "Projects" section's own entry point (V2-T30) — `renderer.ts#main` calls only
 * `wireProjectPanel()`, so that already thousand-line file grows by exactly one import and one
 * call (the task's own "renderer.ts e main.ts não crescem"). This file itself owns no DOM/decision
 * of its own: it wires the five single-responsibility pieces the section is actually made of (PO
